...
cars_ids = []
async def run_parse(search_city,user_percent,user_date):
    headers = {'User-agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/118.0.0.0 Safari/537.36 OPR/104.0.0.0'}
    async with ClientSession(headers=headers) as session:
        ...
        html = await fetch(url, session, params)
        ...
        await asyncio.sleep(random.uniform(1, 3))
        card_text = await fetch(card_url, session, params)
...
async def main():
    ...
    search_city = await search_cities_category(city)
    if search_city is not None:
        ...
        while True:
            ...
            try:
                await run_parse(search_city, percent,user_date)
